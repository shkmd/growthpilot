...
test('project deletion removes only the selected owners project data',()=>{
 ...
 for(const table of ['audits','tasks','workspace_records'])d.exec(`CREATE TABLE ${table}(project_id TEXT,owner TEXT)`);
 ...
 for(const table of ['projects','audits','tasks','workspace_records','google_project_connections','google_project_oauth_states']){
 ...
 for(const table of ['projects','audits','tasks','workspace_records','google_project_connections','google_project_oauth_states'])assert.equal(d.prepare(`SELECT COUNT(*) AS n FROM ${table}`).get().n,2);
 d.close();
});
