...
test('project deletion removes only the selected owners project data',()=>{
 ...
 for(const table of ['google_project_connections','google_project_oauth_states','google_search_sites'])d.exec(`CREATE TABLE ${table}(project_id TEXT,user_id TEXT)`);
 for(const table of ['projects','audits','tasks','workspace_records','crawl_jobs','google_project_connections','google_project_oauth_states','google_search_sites']){
 ...
 for(const table of ['projects','audits','tasks','workspace_records','crawl_jobs','google_project_connections','google_project_oauth_states','google_search_sites'])assert.equal(d.prepare(`SELECT COUNT(*) AS n FROM ${table}`).get().n,2);
 d.close();
});
